...
class AE(nn.Module):
    def __init__(self, conv_layers, encode_dim, input_dim_x, input_dim_y):
        ...
        self.fc_layers_list.append(nn.Linear(conv_layers[-1] * input_dim_x * input_dim_y, encode_dim))

        # add decoder part
        self.fc_layers_list.append(nn.Linear(encode_dim, conv_layers[-1] * input_dim_x * input_dim_y))
        
        self.deconv_layers_list = nn.ModuleList()
        for i in range(len(conv_layers)-1):
            self.deconv_layers_list.append(nn.ConvTranspose2d(conv_layers[-i-1], conv_layers[-i-2], 3, padding=1))
    ...
    def forward(self, x):
        ...
        for lay in self.deconv_layers_list:
            x = lay(x)
...
class Autoencoder:
    def __init__(self, conv_layers, encode_dim, input_dim_x, input_dim_y):
        self.net = AE(conv_layers, encode_dim, input_dim_x, input_dim_y)
